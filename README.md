...
## Дополнительные проекты в репозитории

- **Симулятор нажатия кнопок (1–4 реле + геркон)**: отдельный репозиторий https://github.com/Lepi4/button_simulator_mqtt
...
📋 **Детальная инструкция подключения с фото и схемой:** [docs/relay_module_connection.md](docs/relay_module_connection.md)

Кратко:

- **GPIO23 (ESP32)** → **IN** (релейный модуль)
- **GND (ESP32)** → **GND** (релейный модуль)
- **VCC (модуль)** ← **+5V** (отдельный источник питания)
- **Нагрузка** → **COM/NO** на реле

---

Автор: проект `humidifier_mqtt` — прошивка для ESP32 с поддержкой MQTT и веб-интерфейса.
